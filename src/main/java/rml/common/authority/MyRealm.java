...
public class MyRealm extends AuthorizingRealm {
	...
	protected AuthorizationInfo doGetAuthorizationInfo(PrincipalCollection principals) {
      ...
      //从数据库中获取当前登录用户的详细信息  
      SysUser user = sysUserServiceI.getSysUserByUsername(currentUsername);
//      User user = userService.getByUsername(currentUsername);  
      if(null != user){  
          //实体类User中包含有用户角色的实体类信息  
    	  List<SysRole> roles = sysUserServiceI.getSysRolesBySysUserId(user.getId());
          if(null!=roles && roles.size()>0){  
              ...
              for(SysRole role : roles){  
                  ...
                  List<SysPermission> sysPermissions = sysUserServiceI.getSysPermissionsBySysRoleId(role.getId());
...
//        return null; 
	...
	protected AuthenticationInfo doGetAuthenticationInfo(
        ...
        System.out.println("验证当前Subject时获取到token name:" + token.getUsername());  
        SysUser user = sysUserServiceI.getSysUserByUsername(token.getUsername());
      if(null != user){  
